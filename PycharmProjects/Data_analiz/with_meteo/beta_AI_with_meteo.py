...
data = pd.read_parquet('Atlas_nerm_with_meteo_normalaized.parquet')
...
def create_model(lstm_units, dropout_rate, learning_rate, delta):
    ...
    model.add(Dense(len(target_outputs)))
...
history = model.fit(
    X, y,
    epochs=100,  # Увеличь количество эпох для более качественного обучения
...
with open("model_wm.json", "w") as json_file:
